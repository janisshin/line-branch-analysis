...
# J1 is activated by X1 and X2; it is inhibited by s2
J1: source -> s1; e1 * (((leak_X1 + (1-leak_X1)*((X1/k_X1)/(1+X1/k_X1)))^t_X1) * ((leak_X2 + (1-leak_X2)*((X2/k_X2)/(1+X2/k_X2)))^t_X2) * ((leak_s2 + (1-leak_s2)*(1/(1+s2/k_s2)))^t_s2) * ((k1 * ((source/k_source)^t_source)) - k2*(s1/k_s1)^t_s1)) /((1 + (source/k_source))^t_source + (1+(s1/k_s1))^t_s1 - 1);

# J2 has neither activators nor inhibitors
J2: s1 -> s2; e2 * ((k3*(s1/k_s1)^t_s1)-k4*(s2/k_s2)^t_s2)/((1+s1/k_s1)^t_s1 + (1+s2/k_s2)^t_s2 - 1);

# J3 is inhibited by s1 and X2
J3: s2 -> ; e3 * ((leak_X2 + (1-leak_X2)*(1/(1+(X2/k_X2))))^t_X2) * ((leak_s1 + (1-leak_s1)*(1/(1+(s1/k_s1))))^t_s1) * ((k5*((s2/k_s2)^t_s2) /(1 + s2/k_s2)^t_s2 ));
...
k1 = 5; k2 = 0.001; k3 = 10000; k4 = 0.001; k5 = 10000;
""")
r = te.loada(model)
